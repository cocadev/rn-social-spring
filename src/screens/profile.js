...
export default class UserProfile extends Component {
  ...
  createAnimatedObject = (animatedValue, input, output) => {
    return animatedValue.interpolate({
      inputRange: [Number.MIN_SAFE_INTEGER, input[0], input[1], Number.MAX_SAFE_INTEGER],
  ...
  _renderHeader = (navigation, Me) => {
    return ({ animatedValue }) => {
      ...
      const animatedBottom = this.createAnimatedObject(animatedValue, [0, parallaxHeight], [20, 0]);
      ...
      return (
        <>
          <Animated.View
            leftElement={navigation.getParam('item') && <CloseIcon white navigation={navigation} />}
            rightElement={navigation.getParam('item') ? <RightIcon /> : <EditIcon />}
            profile
            style={[
              styles.bar2,
              {
  ...
  _renderParallaxBackground = ({ animatedValue }) => {
    ...
    return (
      <Animated.View
        style={{ height: animatedHeight, width: '100%', backgroundColor: colors.PURPLE }}
      />
    );
  };
  ...
  render() {
    const { navigation } = this.props;
    const Me = navigation.getParam('item') ? navigation.getParam('item') : POSTS[0];
